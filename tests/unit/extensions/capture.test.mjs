...
describe("collectMediaEvidence", () => {
  it("collects media URLs and never returns a native command", () => {
              ...
              textContent: JSON.stringify({
                "@graph": [
                  ...
                  },
                  {
                    ...
                    caption: "https://cdn.example.com/ld.vtt",
    ...
    assert.ok(urls.includes("https://cdn.example.com/ld.vtt"));
    assert.equal(
      result.evidence.find((item) => item.url === "https://cdn.example.com/ld.vtt")?.kind,
      "subtitle",
    );
    assert.ok(urls.includes("https://cdn.example.com/ld-subs.m3u8"));
    assert.equal(
      result.evidence.find((item) => item.url === "https://cdn.example.com/ld-subs.m3u8")?.kind,
      "subtitle",
    );
    assert.ok(!urls.includes("English closed captions"));
    assert.ok(!urls.includes("https://cdn.example.com/thumb.jpg"));
  ...
  it("collects iframe and link preload media", () => {
      ...
      querySelectorAll: (selector) => {
        ...
        if (selector.includes("a[href]")) {
          return [
            ...
            {
              getAttribute: (name) =>
                name === "href" ? "https://cdn.example.com/clip.m2ts" : null,
            },
            {
              getAttribute: (name) =>
                name === "href" ? "https://cdn.example.com/slash.m2ts/" : null,
            },
          ];
        }
    ...
    assert.ok(urls.includes("https://cdn.example.com/clip.m2ts"));
    assert.ok(urls.includes("https://cdn.example.com/slash.m2ts/"));
