...
Display name is WebMedia DL. Repository and CLI are `webmedia-dl`. Python package is `webmedia_dl`. Swift prefix is `WebMediaDL`. `wmdl` is a personal alias only and MUST NOT be the public console script.
