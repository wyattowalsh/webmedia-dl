...
### Requirement: diagnostics support follows the shared typed model

The `diagnostics-support` surface SHALL use the shared job, event, capability, policy,
...
- **WHEN** a `diagnostics-support` payload is produced
- **THEN** it validates against the corresponding JSON Schema in `schemas/`

### Requirement: Policy and evidence gates

`diagnostics-support` SHALL honor client and worker policy profiles, SHALL refuse DRM
...
Failures in `diagnostics-support` SHALL write only to staging or durable queue state
