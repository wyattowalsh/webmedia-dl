...
### Requirement: cross device workers follows the shared typed model

The `cross-device-workers` surface SHALL use the shared job, event, capability, policy,
...
- **WHEN** a `cross-device-workers` payload is produced
- **THEN** it validates against the corresponding JSON Schema in `schemas/`

### Requirement: Policy and evidence gates

`cross-device-workers` SHALL honor client and worker policy profiles, SHALL refuse DRM
...
Failures in `cross-device-workers` SHALL write only to staging or durable queue state
