...
### Requirement: live manifest recording follows the shared typed model

The `live-manifest-recording` surface SHALL use the shared job, event, capability, policy,
...
- **WHEN** a `live-manifest-recording` payload is produced
- **THEN** it validates against the corresponding JSON Schema in `schemas/`

### Requirement: Policy and evidence gates

`live-manifest-recording` SHALL honor client and worker policy profiles, SHALL refuse DRM
...
Failures in `live-manifest-recording` SHALL write only to staging or durable queue state
