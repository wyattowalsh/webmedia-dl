...
### Requirement: apple system integrations follows the shared typed model

The `apple-system-integrations` surface SHALL use the shared job, event, capability, policy,
...
- **WHEN** a `apple-system-integrations` payload is produced
- **THEN** it validates against the corresponding JSON Schema in `schemas/`

### Requirement: Policy and evidence gates

`apple-system-integrations` SHALL honor client and worker policy profiles, SHALL refuse DRM
...
Failures in `apple-system-integrations` SHALL write only to staging or durable queue state
