...
| Complete-client share http-direct | PASS | iPhone/iPad/visionOS share adapters call `saveIfDirect` before loopback POST; share sheets restore the App Group Files bookmark via `WebMediaDLShareIntake.fromSavedBookmark`; Mac share stays worker-only; watch/tv still lack `WebMediaDLHttpDirect` |
...
| Swift Core CI job | PASS | GitHub Actions `ci` run `32549087599` on `fea26f1`: `IdentityTests` executed 6 tests, 0 failures on `macos-15` |
| Swift Core contract tests | PASS | `ContractTests.swift` executed 12 tests, 0 failures with IdentityTests (6) on GitHub `macos-15` run `32549087599` (`fea26f1`) |
| Apple package compile CI | PASS | GitHub Actions `ci` run `32549087599` on `fea26f1`: Core `swift test` 18 tests, 0 failures (`ContractTests` 12 + `IdentityTests` 6); Safari handler `swiftc -typecheck`; 8× `BUILD SUCCEEDED`. Device runtime stays BLOCKED |
...
| Safari wrapping / signed NSExtension | BLOCKED | `swiftc -typecheck` of `SafariWebExtensionHandler.swift` executed on GitHub `macos-15` run `32549087599`; signed Xcode NSExtension wrapping is not executed |
