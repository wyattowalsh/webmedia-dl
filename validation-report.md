...
|---|---|---|
| `uv run pytest` | PASS | 549 tests on GitHub Actions `ci` run `32554681071` (`fc10e4d`); local follow-up covers empty-provider "produced no source files", `output_paths` without a primary path, queue zero-UUID events, and `wmdl` not being the console script |
| `uv run pytest --cov` | PASS | 99.89% on `fc10e4d`; local 100% line/branch (`fail_under` 99) |
...
| Swift Core CI job | PASS | GitHub Actions `ci` run `32554681071` on `fc10e4d`: `IdentityTests` executed 6 tests, 0 failures on `macos-15` |
| Swift Core contract tests | PASS | `ContractTests.swift` executed 12 tests, 0 failures with IdentityTests (6) on GitHub `macos-15` run `32554681071` (`fc10e4d`) |
| Apple package compile CI | PASS | GitHub Actions `ci` run `32554681071` on `fc10e4d`: Core `swift test` 18 tests, 0 failures (`ContractTests` 12 + `IdentityTests` 6); Safari handler `swiftc -typecheck`; 8× `BUILD SUCCEEDED`. Device runtime stays BLOCKED |
| OpenSpec scenarios | PASS | every capability spec scenario has WHEN/THEN; each scenario title maps to a named Python or Swift test in a pinned source file; popup markup is parsed; drop records `local_path`; local submit does not upload; job submit forbids native argv; queue-level events use a zero UUID; `wmdl` is not the console script |
...
| Safari wrapping / signed NSExtension | BLOCKED | `swiftc -typecheck` of `SafariWebExtensionHandler.swift` executed on GitHub `macos-15` run `32554681071`; signed Xcode NSExtension wrapping is not executed |
