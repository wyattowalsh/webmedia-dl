...
| Apple package compile CI | PASS | GitHub Actions `ci` run `32572904900` on `7981aaa`: Core `swift test` 18 tests, 0 failures (`ContractTests` 12 + `IdentityTests` 6); Safari handler `swiftc -typecheck`; 8× package `BUILD SUCCEEDED` plus 4× unsigned `com.apple.product-type.app-extension` `.appex`. Device runtime stays BLOCKED |
